...
export default function App() {
  ...
  const [chatMessages, setChatMessages] = useState([
    { role: 'ai', text: 'សួស្តី! ខ្ញុំជា Smart Map AI។ តើលោកអ្នកចង់ដឹងព័ត៌មានពីទីតាំងណាមួយទេ?' }
  ]);
  ...
  // Fetch POIs from OpenStreetMap robustly
  const fetchNearbyPOIs = async (lat, lng) => {
      ...
      const query = `
        [out:json][timeout:15];
        (
          node["amenity"~"school|hospital|police|fire_station|clinic|kindergarten"](around:4000,${lat},${lng});
          node["office"~"government|administrative"](around:4000,${lat},${lng});
          node["place"~"village|townhall"](around:4000,${lat},${lng});
        );
        out body;
      `;
      try {
          // Direct fetch first to avoid proxy limits
          ...
          if (data && data.elements && data.elements.length > 0) {
              const formattedPOIs = data.elements.filter(e => e.tags && e.tags.name).map(el => {
                  ...
                  if (['school', 'kindergarten'].includes(amenity)) type = "សាលារៀន";
                  else if (['hospital', 'clinic'].includes(amenity)) type = "មន្ទីរពេទ្យ / គ្លីនិក";
                  else if (['police', 'fire_station'].includes(amenity)) type = "ប៉ុស្តិ៍ប៉ូលីស";
                  else if (['government', 'administrative', 'village', 'townhall'].includes(amenity)) type = "សាលាឃុំ / ផ្ទះមេភូមិ";
  ...
  const initializeMap = async () => {
    ...
    if (navigator.geolocation) {
      watchIdRef.current = navigator.geolocation.watchPosition(
        ...
        (error) => { 
            console.warn("GPS Tracking info:", error.message);
  ...
  // Handling Search Change & Auto-complete
  ...
  // Handle Enter Key / Submit Search Form
  const handleSearchSubmit = (e) => {
    ...
    if (searchResults.length > 0) {
       ...
       // Fallback geocoding if Google Places is slow
       fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(searchQuery)}&countrycodes=kh&limit=1&accept-language=km,en`)
        .then(res => res.json())
        .then(data => {
            if(data && data.length > 0) {
               if(!map) return;
               const loc = new window.google.maps.LatLng(parseFloat(data[0].lat), parseFloat(data[0].lon));
               map.panTo(loc); map.setZoom(15);
               fetchNearbyPOIs(loc.lat(), loc.lng());
               setShowSearchDropdown(false);
               if(window.innerWidth < 768) setIsSidebarOpen(false);
            } else {
               showToast("មិនរកឃើញទីតាំងនេះទេ", "error");
            }
  ...
  // Merge Admin Data with OSM Data seamlessly (Combine nearby, prioritize Admin)
  const allLocationsForMap = useMemo(() => {
    if (!firebaseLocations && !osmLocations) return [];
    
    // Start with all Firebase (Admin) locations
    const mergedList = [...firebaseLocations];

    // Add OSM locations only if they are NOT close to an existing Admin location
    osmLocations.forEach(osmLoc => {
        const isDuplicate = firebaseLocations.some(fbLoc => 
            calculateDistance(osmLoc.lat, osmLoc.lng, fbLoc.lat, fbLoc.lng) < 0.1 // within 100 meters
        );
        if (!isDuplicate) {
            mergedList.push(osmLoc);
        }
    });
  ...
  const filteredAndSortedLocations = useMemo(() => {
      ...
      // Sort: Admin Data first, then by distance
      return mappedLocs.sort((a, b) => {
          if (a.isAdminData && !b.isAdminData) return -1;
          if (!a.isAdminData && b.isAdminData) return 1;
  ...
  const handleDeleteLocation = async (locId) => {
     try {
         ...
         showToast("មិនអាចលុបបានទេ សូមព្យាយាមម្តងទៀត", "error");
     }
  };
  ...
  // Robust AI Chat integration with Gemini 2.5 API structure
  const handleAiChatSubmit = async (e) => {
    ...
    try {
      const apiKey = ""; // Keep empty, runtime handles it
      ...
      const payload = {
        ...
        systemInstruction: { parts: [{ text: "អ្នកគឺជាជំនួយការ Smart Map ប្រទេសកម្ពុជា។ ឆ្លើយតបជាភាសាខ្មែរខ្លីៗ គោរព និងពន្យល់ពីទីតាំងភូមិសាស្រ្តបានច្បាស់លាស់។" }] }
      };
      ...
      const aiResponse = data.candidates?.[0]?.content?.parts?.[0]?.text || "សូមអភ័យទោស ខ្ញុំមិនយល់សំណួរនេះទេ។";
      setChatMessages(prev => [...prev, { role: 'ai', text: aiResponse }]);
    } catch (error) {
      console.error(error);
      setChatMessages(prev => [...prev, { role: 'ai', text: "មានបញ្ហាភ្ជាប់ទៅកាន់ប្រព័ន្ធ AI។ សូមព្យាយាមម្តងទៀត។" }]);
  ...
  return (
    // Applied touch-none and overscroll-none to act like native app on mobile
    <div className={`fixed inset-0 h-[100dvh] w-full flex flex-col font-sans overflow-hidden overscroll-none transition-colors duration-300 ${isDarkMode ? 'dark bg-gray-950 text-white' : 'bg-gray-50 text-gray-900'}`}>
      ...
      <header className="h-16 flex items-center justify-between px-4 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm z-30 shrink-0">
        ...
        {/* Google Maps Search Bar (Desktop) - Now Wrapped in Form */}
        <div ref={searchContainerRef} className="hidden md:flex relative flex-1 max-w-xl mx-6">
          <form onSubmit={handleSearchSubmit} className="w-full relative">
            ...
            />
            <button type="submit" className="absolute left-4 top-3"><Search className="w-4.5 h-4.5 text-gray-400 hover:text-blue-500 transition-colors" /></button>
      ...
      <div className="md:hidden p-3 bg-white dark:bg-gray-900 border-b dark:border-gray-800 z-30 shrink-0 shadow-sm" ref={searchContainerMobileRef}>
         <form onSubmit={handleSearchSubmit} className="relative">
            ...
            <button type="submit" className="absolute left-3.5 top-3"><Search className="w-4 h-4 text-gray-400 hover:text-blue-500" /></button>
        ...
        <aside className={`fixed md:relative bottom-0 left-0 right-0 md:right-auto w-full md:w-[350px] h-[65vh] md:h-full bg-white dark:bg-gray-900 border-t md:border-t-0 md:border-r border-gray-200 dark:border-gray-800 z-40 flex flex-col transition-transform duration-300 ease-in-out shadow-[0_-10px_40px_rgba(0,0,0,0.1)] md:shadow-none rounded-t-3xl md:rounded-none ${isSidebarOpen ? 'translate-y-0 md:translate-x-0' : 'translate-y-full md:-translate-x-full'}`}>
          ...
          {isAdmin ? (
            ...
            <div className="flex flex-col h-full overflow-hidden">
               ...
               {/* Location List */}
               <div className="flex-1 overflow-y-auto overscroll-contain p-4 space-y-3 pb-12">
               ...
               {isFetchingPois && filteredAndSortedLocations.length === 0 ? (
                  ...
                  <div key={loc.id} onClick={() => focusLocation(loc)} className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 p-4 rounded-2xl cursor-pointer hover:border-blue-500 hover:shadow-md transition-all group relative overflow-hidden">
                     ...
                     <div className="w-full">
                        ...
                        {/* Admin Details Snippet in List - Prioritized visually */}
                        {loc.isAdminData && loc.phone && (
                           <div className="mt-3 bg-emerald-50 dark:bg-emerald-900/20 p-3 rounded-xl border border-emerald-100 dark:border-emerald-900/50">
                              ...
                              <div className="flex gap-2 mt-2.5">
                                 <button onClick={(e) => { e.stopPropagation(); handleCall(loc); }} className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white py-2 rounded-lg text-xs font-extrabold transition-colors shadow-sm">
                                    ចុចខលឥឡូវនេះ
                                 </button>
                                 {/* Admin Delete Button visible right here for easy management */}
                                 {isAdmin && (
                                     <button onClick={(e) => { e.stopPropagation(); handleDeleteLocation(loc.id); }} className="px-3 bg-rose-100 text-rose-600 hover:bg-rose-200 dark:bg-rose-900/40 dark:text-rose-400 rounded-lg transition-colors flex items-center justify-center">
